################################################################################
# ROSMAP (Real Data) Analysis as Example — pseudo-bulk feature construction + screening +
...
#
# PURPOSE
#   This script applies the QuasiMed workflow to ROSMAP vasculature scRNA-seq data:
...
#   Filters subjects/genes -> builds M/F -> runs QuasiMed -> returns results
################################################################################
run_singlecell_model <- function(min_Cells = 30,
  ...
  ################################################################################
  # Section 6: QuasiMed on ROSMAP data
  #
  ...
  # Run QuasiMed once for this filtered/constructed dataset
  ROSMAPresults <- run_quasimed(data.name = rosmap_realdat, n_genes = n_genes, covariate.names = "Z")
